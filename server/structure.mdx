...
Authentication handler classes, one per security scheme found in your spec. See [Authentication](/server/authentication) for details on each type.
...
<Columns cols={2}>
  <Card title="Authentication" icon="lock" iconType="sharp-solid" href="/server/authentication">
    Configure credentials for each auth type.
  </Card>
